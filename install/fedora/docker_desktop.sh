...
# This script automates the installation and configuration of Docker Desktop on Fedora workstations
# Based on official Docker documentation: https://docs.docker.com/desktop/setup/install/linux/fedora/
#
# Usage: ./install_docker_desktop_fedora.sh [OPTIONS]
# Options:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --manual-setup)
            ...
            ;;
        --help)
            echo "Docker Desktop Installation Script for Fedora"
            echo
...
check_fedora_version() {
    ...
    if ! command -v dnf &> /dev/null; then
        log_error "This script is designed for Fedora. DNF package manager not found."
        exit 1
    fi
    ...
    if [[ $FEDORA_VERSION -lt 41 ]]; then
        log_warning "Docker Desktop officially supports Fedora 41 and 42. You're running Fedora $FEDORA_VERSION."
...
check_existing_docker() {
    ...
    local docker_packages=$(rpm -qa | grep -E '^docker-' | head -5)
...
download_docker_desktop() {
    ...
    DOWNLOAD_PATH="/tmp/docker-desktop-x86_64.rpm"

    # Download Docker Desktop RPM
    if [[ -f "$DOWNLOAD_PATH" ]]; then
        log_info "Docker Desktop RPM already exists, removing old version..."
        rm -f "$DOWNLOAD_PATH"
    fi
...
install_docker_desktop() {
    ...
    rm -f "$DOWNLOAD_PATH"
    log_info "Cleaned up downloaded RPM file"
}
...
show_post_install_info() {
    ...
    echo "https://docs.docker.com/desktop/setup/install/linux/fedora/"
    echo
}
